...
def create_dataset(opt):
    ...
    if opt.dataset_type == 'remote_sensing_images':
        ...
        if opt.source_domain == 'Amazon_RO':
            ...
            opt.reference_t1_name = opt.source_reference_name_T1
            opt.reference_t2_name = opt.source_reference_name_T2
            opt.buffer_dimension_in = opt.source_buffer_in
            opt.buffer_dimension_out = opt.source_buffer_out
        ...
        if opt.target_domain == 'Amazon_RO':
            ...
            opt.reference_t1_name = opt.target_reference_name_T1
            opt.reference_t2_name = opt.target_reference_name_T2
            opt.buffer_dimension_in = opt.target_buffer_in
            opt.buffer_dimension_out = opt.target_buffer_out
        ...
        # source = dataset_s
        # target = dataset_t
        # dataloader_s = dataset.dataloader_s
        # dataloader_t = dataset.dataloader_t
        
    return dataset, scalers # mog: usar isso se for rodar 'train_remote_sensing.py'
    
    # return source, target, dataloader_s, dataloader_t, scalers
    # mog: usar isso/\ se for rodar 'mog_train.py'
...
class RemoteSensingCustomDatasetDataLoader():
    ...
    def __init__(self, dataset_s, dataset_t, opt):
        ...
        if self.opt.phase == 'train':
            if self.opt.use_tile_configuration:
                mask_s = self.mask_creation(dataset_s.images_norm[0].shape[0], dataset_s.images_norm[0].shape[1], dataset_s.horizontal_blocks, dataset_s.vertical_blocks, dataset_s.Train_tiles, dataset_s.Valid_tiles, dataset_s.Undesired_tiles)
            else:
            ...
            if self.opt.use_tile_configuration:
                print("no multiplier", patch_coordinates_s.shape)
                aug_index, patch_coordinates_s = self.augmentations_multiplier_index(patch_coordinates_s)
                # print("aug_index", aug_index.shape)
                print("with multiplier", patch_coordinates_s.shape)
                zeros = np.zeros((patch_coordinates_s.shape[0], patch_coordinates_s.shape[1]+1))
                zeros[:,:2] =  patch_coordinates_s
                zeros[:,2] = aug_index
                patch_coordinates_s = zeros
                # print("new_shape=", patch_coordinates_s.shape)
                # print(patch_coordinates_s[:])
            ...
            if self.opt.use_tile_configuration and size_s > size_t:
                np.random.shuffle(patch_coordinates_s)
                patch_coordinates_s = patch_coordinates_s[:size_t]
                print("patch_coordinates_s =", patch_coordinates_s.shape)
            ...
            size_s2 = patch_coordinates_s.shape[0]
            size_t2 = patch_coordinates_t.shape[0]
        ...
        if self.opt.same_coordinates:
            ...
            # Performing a shuffle operation in order to get different pairs of samples
    ...
    def mask_creation(self, mask_row, mask_col, num_patch_row, num_patch_col, Train_tiles, Valid_tiles, Undesired_tiles):
        train_index = 1
        teste_index = 2
        valid_index = 3
        undesired_index = 4
    ...
    def augmentations_multiplier_index(self, coordinates):
        ...
        for i in range(size):
            ...
            # print(aug_index[a:b])
        new_coor = self.replicate_coordinates(coordinates, step)
        return aug_index, new_coor

    def replicate_coordinates(self, coordinates, step):
        size = len(coordinates)
