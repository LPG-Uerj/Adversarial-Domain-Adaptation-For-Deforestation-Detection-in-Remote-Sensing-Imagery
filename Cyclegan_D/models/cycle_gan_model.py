...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            # Tnning parameter of the difference loss
            ...
            parser.add_argument('--lambda_task', type=float, default=0.5, help='Learning rate for Task Loss.')
        return parser
    ...
    def __init__(self, opt):
        ...
        if self.isTrain :
            ...
            if self.opt.use_task_loss:
                visual_names_A.append('A_class_ref')
        ...
        if self.isTrain:
            ...
            if self.opt.use_task_loss:
                self.model_names.append('deep_lab_v3p')
        ...
        if self.isTrain:
            ...
            self.criterionDif = torch.nn.L1Loss()
        ...
        if opt.use_task_loss:
            self.netdeep_lab_v3p = DeepLabV3plus.create(opt).to(self.device)
            class_weights = torch.FloatTensor(opt.task_weights).to(self.device)
            self.criterionTask = DeepLabV3plus.FocalLoss(weight=class_weights, gamma=opt.task_gamma)
            self.optimizer_T = torch.optim.Adam(self.netdeep_lab_v3p.parameters(), lr=opt.task_lr)
    ...
    def set_input(self, input):
        ...
        if self.opt.dataset_type == 'remote_sensing_images':
            ...
            if self.opt.phase == 'train':
                ...
                if self.opt.use_task_loss:
                    self.A_class_ref = input['A_class_ref'].to(self.device)
                    # self.B_clas_ref = input['B_clas_ref' if AtoB else 'A_clas_ref'].to(self.device)
    ...
    def backward_G(self):
        ...
        lambda_task = self.opt.lambda_task
        ...
        if lambda_idt_t > 0:
            ...
            self.loss_idt_A += self.loss_idt_A_t
            self.loss_idt_B += self.loss_idt_B_t
        ...
        if lambda_idt_s > 0:
            ...
            self.loss_idt_A += self.loss_idt_A_s
            self.loss_idt_B += self.loss_idt_B_s       
        ...
        size = len(self.real_diff_A)
        self.real_diff_A_norm = self.real_diff_A/self.real_diff_A_.view(size,1,1,1)  # (A_2 - A_1)/Norm2(A_2 - A_1)
        self.real_diff_B_norm = self.real_diff_B/self.real_diff_B_.view(size,1,1,1)  # (B_2 - B_1)/Norm2(B_2 - B_1)
        self.diff_A_norm = self.diff_A/self.diff_A_.view(size,1,1,1)     # (G_A(A_2) - G_A(A_1))/Norm2(G_A(A_2) - G_A(A_1))
        self.diff_B_norm = self.diff_B/self.diff_B_.view(size,1,1,1)
        ...
        # self.diff_B_norm = self.diff_B/self.diff_B_ 
        ...
        if self.opt.use_task_loss:
            predict_A = self.netdeep_lab_v3p(self.fake_B)
            self.loss_task = self.criterionTask(predict_A, self.A_class_ref) * lambda_A * lambda_task
            self.loss_G += self.loss_task
